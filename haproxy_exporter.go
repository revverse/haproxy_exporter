...
func NewExporter(uri string, sslVerify bool, selectedServerMetrics map[int]*prometheus.Desc, timeout time.Duration, nbproc int, logger log.Logger) (*Exporter, error) {
	u, err := url.Parse(uri)
	if err != nil {
		return nil, err
	}
...
func (e *Exporter) SumRows(cbody io.ReadCloser) (rows [][]string) {
	i := 0
	need_to_sum := []int{4,7,8,9,10,12,13,14,15,16,33,39,40,41,42,43,44,48,51,52,53,54,79}
	rows = rows[:0][:0]
	...
	for {
		...
		if err != nil {
			level.Info(e.logger).Log("msg", "Can't read Row", "err", err)
			//level.Error(e.logger).Log("msg", "Can't read Row, "err", err)
...
func (e *Exporter) scrape(ch chan<- prometheus.Metric) (up float64) {
	...
	var newData string
	for _, value := range e.SumRows(body) {
...
func main() {
	...
	exporter, err := NewExporter(*haProxyScrapeURI, *haProxySSLVerify, selectedServerMetrics, *haProxyTimeout, *haProxyNbProc, logger)
	if err != nil {
